...
1.  Utiliser une boucle de manière à faire une boîte à moustaches par variable quantitative `r colnames(iris)[1:4]` selon l’espèce `r colnames(iris)[5]`. Le mieux est de définir un objet liste dans lequel chacun des graphiques sera stocké et ensuite on utilisera la fonction ***grid.arrange*** du package gridExtra
...
```{r eval=T,fig.height=5,fig.width=10,fig.align='center',fig.cap="Trois graphes différents"}
#| code-fold: true
#| code-summary: "Voir la correction"
graph<-list()
...
3.  Réaliser le graphe suivant qui réunit les trois précédents sur un même graphique (on utilisera la fonction geom_smooth avec method="lm").
